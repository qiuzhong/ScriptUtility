...
OFFICIAL_DEST_BINARY_DIR="/mnt/otcqa/NewSampleApp/android/${BRANCH}/${CROSSWALK_VERSION}/32bit"
...
init_opts() {
    ...
    if [ ! -d ${ROOT_DIR}/crosswalk ]; then
        cp -a ${CROSSWALK} ${ROOT_DIR}/crosswalk-${CROSSWALK_VERSION}.zip
        unzip ${ROOT_DIR}/crosswalk-${CROSSWALK_VERSION}.zip -d ${ROOT_DIR}
        mv -fv ${ROOT_DIR}/crosswalk-${CROSSWALK_VERSION} ${ROOT_DIR}/crosswalk
...
update_sampeapp_config() {
    ...
    sed -i "s|8.37.189.14|${CROSSWALK_VERSION}|" ${ROOT_DIR}/crosswalk-samples/extensions-android/xwalk-echo-extension-src/build.xml
...
build_apk() {
    ...
    ./build.sh ${mode} ${arch}
    mv -fv $ROOT_DIR/crosswalk-samples/extensions-android/xwalk-echo-extension-src/lib/crosswalk-${CROSSWALK_VERSION}/*.apk $BINARY_DIR
    cd $BINARY_DIR
