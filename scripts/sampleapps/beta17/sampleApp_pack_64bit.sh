...
SAMPLEAPP_DIR="/home/orange/01_qiuzhong/myworkspace/sampleapp-17"
DEST_BINARY_DIR=""
...
OFFICIAL_DEST_BINARY_DIR="/mnt/otcqa/NewSampleApp/android/${BRANCH}/${CROSSWALK_VERSION}/64bit"
...
init_opts() {
    ...
    if [ ! -d ${ROOT_DIR}/crosswalk ]; then
        cp -a ${CROSSWALK} ${ROOT_DIR}/crosswalk-${CROSSWALK_VERSION}-64bit.zip
        unzip ${ROOT_DIR}/crosswalk-${CROSSWALK_VERSION}-64bit.zip -d ${ROOT_DIR}
        mv -fv ${ROOT_DIR}/crosswalk-${CROSSWALK_VERSION}-64bit ${ROOT_DIR}/crosswalk
        CROSSWALK_DIR=${ROOT_DIR}/crosswalk
    fi    
}
...
update_sampeapp_config() {
    ...
    sed -i "s|8.37.189.14|${CROSSWALK_VERSION}-64bit|" ${ROOT_DIR}/crosswalk-samples/extensions-android/xwalk-echo-extension-src/build.xml
...
build_apk() {
    ...
    ./build.sh ${mode} ${arch}
    mv -fv $ROOT_DIR/crosswalk-samples/extensions-android/xwalk-echo-extension-src/lib/crosswalk-${CROSSWALK_VERSION}-64bit/*.apk ${BINARY_DIR}
    cd ${BINARY_DIR}

    # Hexgl:
    python $CROSSWALK_DIR/make_apk.py --package=org.xwalk.hexgl --name=Hexgl --app-root=$ROOT_DIR/crosswalk-samples/HexGL --app-local-path=index.html --mode=$1 --arch=$2 --enable-remote-debugging
    
    # Memorygame:
    python $CROSSWALK_DIR/make_apk.py --package=org.xwalk.memorygame --manifest=$ROOT_DIR/crosswalk-samples/MemoryGame/src/manifest.json --mode=$1 --arch=$2 --enable-remote-debugging
    ...
    python $CROSSWALK_DIR/make_apk.py --package=org.xwalk.spacedodgegame2 --manifest=$ROOT_DIR/crosswalk-samples/space-dodge-game/manifest-orientation-resize/manifest.json --app-versionCode=1 --mode=$1 --arch=$2 --enable-remote-debugging

    python $CROSSWALK_DIR/make_apk.py --package=org.xwalk.spacedodgegame3 --manifest=$ROOT_DIR/crosswalk-samples/space-dodge-game/manifest-orientation-scale/manifest.json --app-versionCode=1 --mode=$1 --arch=$2 --enable-remote-debugging

    python $CROSSWALK_DIR/make_apk.py --package=org.xwalk.spacedodgegame4 --manifest=$ROOT_DIR/crosswalk-samples/space-dodge-game/screen-orientation-resize/manifest.json --app-versionCode=1 --mode=$1 --arch=$2 --enable-remote-debugging

    python $CROSSWALK_DIR/make_apk.py --package=org.xwalk.spacedodgegame5 --manifest=$ROOT_DIR/crosswalk-samples/space-dodge-game/screen-orientation-scale/manifest.json --app-versionCode=1 --mode=$1 --arch=$2 --enable-remote-debugging
