...
SAMPLEAPP_DIR="/home/orange/01_qiuzhong/myworkspace/sampleapp"
DEST_BINARY_DIR=""
TEST_DEST_BINARY_DIR="${ROOT_DIR}/xwalk_sampleapp"
OFFICIAL_DEST_BINARY_DIR="/mnt/otcqa/NewSampleApp/android/master/18.46.474.0/64bit"
...
update_sampeapp_config() {
    ...
    sed -i "s|--targets=\$ARCH||g" ${ROOT_DIR}/crosswalk-samples/extensions-android/build.sh
}
...
build_apk() {
    ...
    # 2. HangOnMan    
    $PKG_TOOLS --crosswalk=$CROSSWALK --platforms=android --android=${mode}  --enable-remote-debugging ${ROOT_DIR}/crosswalk-samples/HangOnMan/src

    # 3. hello-world
    $PKG_TOOLS --crosswalk=$CROSSWALK --platforms=android --android=${mode}  --enable-remote-debugging ${ROOT_DIR}/crosswalk-samples/hello-world

    # 4. HexGL
    if [ -f ${ROOT_DIR}/crosswalk-samples/HexGL/manifest.json ]; then
        $PKG_TOOLS --crosswalk=$CROSSWALK --platforms=android --android=${mode}  --enable-remote-debugging ${ROOT_DIR}/crosswalk-samples/HexGL
    else
        $PKG_TOOLS --crosswalk=$CROSSWALK --manifest='{ "name": "HexGL", 
                                                      ...
                                                      }' --platforms=android --android=${mode}  --enable-remote-debugging ${ROOT_DIR}/crosswalk-samples/HexGL
    fi

    # 5. MemoryGame
    $PKG_TOOLS --crosswalk=$CROSSWALK --platforms=android --android=${mode}  --enable-remote-debugging ${ROOT_DIR}/crosswalk-samples/MemoryGame/src

    # 6. simd-mandelbrot
    $PKG_TOOLS --crosswalk=$CROSSWALK --platforms=android --android=${mode}  --enable-remote-debugging ${ROOT_DIR}/crosswalk-samples/simd-mandelbrot

    # 7. space-dodge-game
    $PKG_TOOLS --crosswalk=$CROSSWALK --platforms=android --android=${mode}  --enable-remote-debugging ${ROOT_DIR}/crosswalk-samples/space-dodge-game/base

    # 7.1 
    $PKG_TOOLS --crosswalk=$CROSSWALK --platforms=android --android=${mode}  --enable-remote-debugging  ${ROOT_DIR}/crosswalk-samples/space-dodge-game/manifest-orientation-resize

    # 7.2
    $PKG_TOOLS --crosswalk=$CROSSWALK --platforms=android --android=${mode}  --enable-remote-debugging  ${ROOT_DIR}/crosswalk-samples/space-dodge-game/manifest-orientation-scale

    # 7.3
    $PKG_TOOLS --crosswalk=$CROSSWALK --platforms=android --android=${mode}  --enable-remote-debugging  ${ROOT_DIR}/crosswalk-samples/space-dodge-game/screen-orientation-resize

    # 7.4
    $PKG_TOOLS --crosswalk=$CROSSWALK --platforms=android --android=${mode}  --enable-remote-debugging  ${ROOT_DIR}/crosswalk-samples/space-dodge-game/screen-orientation-scale

    # 8. webgl
    $PKG_TOOLS --crosswalk=$CROSSWALK --platforms=android --android=${mode}  --enable-remote-debugging ${ROOT_DIR}/crosswalk-samples/webgl

    # 9. webrtc
    $PKG_TOOLS --crosswalk=$CROSSWALK --platforms=android --android=${mode}  --enable-remote-debugging ${ROOT_DIR}/crosswalk-samples/webrtc/client
